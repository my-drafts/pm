...
class Cell extends Gameable{
	...
	bind(gist){
		if(gist instanceof Gist) this._gists.push(gist);
		else throw '[Cell.bind]: Wrong gist';
	}
	unbind(gist){
		if(gist instanceof Gist) this._gists = this._gists.filter(function(g){ return g!=gist; });
		else throw '[Cell.unbind]: Wrong gist';
	}
	gists(is){ return this._gists.filter(function(g){ return g.is==String(is).toLowerCase(); }); }
